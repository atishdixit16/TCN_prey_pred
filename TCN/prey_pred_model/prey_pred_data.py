...
def prey_pred_data_randinit(examples, seq_length, yinit = [80,50], t_range = (0.0, 200), add_dt = False):
    ...
    for i in range(batch_no):
        y0, y1 = u_min + (u_max-u_min)*np.random.rand(1), u_min + (u_max-u_min)*np.random.rand(1)
        batch_data = prey_pred_data(batch_samples, seq_length, yinit = [y0.item() , y1.item()], add_dt= add_dt)
    ...
    if examples % batch_no:
        batch_data = prey_pred_data( (examples % batch_no), seq_length, yinit = yinit, add_dt= add_dt)
        batch_data = np.concatenate( ( batch_data[0], batch_data[1].reshape(batch_samples, 2 , 1) ) , axis=2 )
